...
test('people profiles live in an animated fit-content dashboard tab control', () => {
  ...
  assert.match(html, /@media \(max-width: 760px\) \{[\s\S]*\.trip-tabs \{[^}]*position: fixed[^}]*bottom: max\(14px, calc\(env\(safe-area-inset-bottom\) \+ 14px\)\)[^}]*left: 50%[^}]*transform: translate\(-50%, 25px\)[^}]*z-index: 30[^}]*pointer-events: auto/);
  ...
  assert.match(html, /'Tanner':\{floor:'1st floor', apartment:'1st floor apartment', bed:'Queen bed', room:'Queen room', roommates:\['David'\]\}/);
  assert.match(html, /'Candace':\{floor:'3rd floor', apartment:'3rd floor apartment', bed:'Twin bed', room:'2-twin room', roommates:\['Erika'\]\}/);
  assert.match(html, /'Jacob G\.':\{floor:'2nd floor', apartment:'2nd floor apartment', bed:'Queen bed', room:'Queen room', roommates:\['Jacob M\.'\]\}/);
  assert.match(html, /'Amanda':\{floor:'2nd floor', apartment:'2nd floor apartment', bed:'Twin bed', room:'2-twin room', roommates:\['Kait'\]\}/);
  ...
  assert.match(html, /function roomLine\(person\)/);
  assert.match(html, /function roomSummary\(person\)/);
  assert.match(html, /function phoneLine\(person\)/);
  assert.match(html, /function arrivalCountdownLine\(person, board\)/);
  assert.match(html, /Room TBD/);
  assert.match(html, /Arrival TBD/);
  assert.match(html, /No dinner claimed yet\./);
  assert.match(html, /id="profileArrival">Arrives TBD<\/span>/);
  assert.match(html, /id="profilePhone">No phone added yet\.<\/span>/);
  assert.match(html, /id="profileRoom"/);
  assert.match(html, /id="profileDinner"/);
  ...
  assert.match(html, /<span class="person-room">\$\{escapeHtml\(roomSummary\(person\)\)\}<\/span>/);
  assert.match(html, /<span>\$\{escapeHtml\(profileSummary\(person, board, dinner\)\)\}<\/span>/);
  assert.match(html, /function openProfileDetail\(name\)/);
  assert.match(html, /profileArrival\.textContent=arrivalCountdownLine\(name, board\)/);
  assert.match(html, /profilePhone\.innerHTML=phoneLine\(name\)/);
  assert.match(html, /profileRoom\.textContent=roomLine\(name\)/);
