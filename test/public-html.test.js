...
test('dashboard itinerary cards use subtle expand icons to open an iCal-style day window', () => {
  ...
  assert.doesNotMatch(html, /const visibleEventLimit=2/);
  assert.match(html, /const needsExpand=dayEvents\.length>1/);
...
test('calendar day cards show per-user unseen custom event badges that clear when the day is opened', () => {
  ...
  assert.match(html, /return `<span class="calendar-unseen-badge" aria-label="\$\{count\} unseen calendar update\$\{count===1\?'':'s'\}">\$\{count\}<\/span>`/);
  assert.doesNotMatch(html, /calendar-unseen-badge is-dot/);
  ...
  assert.match(html, /\.calendar-unseen-badge \{[^}]*background: rgba\(41, 143, 184, \.25\);[^}]*border: 1px solid rgba\(255,255,255,\.12\);[^}]*box-shadow: 0 3px 8px rgba\(5,34,48,\.12\);/s);
...
test('unseen custom events keep a subtle dot on the event block after opening the day', () => {
  ...
  assert.match(html, /\.event-unseen-dot \{[^}]*background: rgba\(41, 143, 184, \.25\);[^}]*box-shadow: 0 3px 8px rgba\(5,34,48,\.12\);/s);
  assert.match(html, /\$\{unseenEventBlockDot\(event\)\}<strong>\$\{escapeHtml\(event\.title\)\}<\/strong>/);
  assert.match(html, /function clearEventUnseenDot\(eventId, element\)/);
  assert.match(html, /activeDayUnseenEventIds\.delete\(eventId\)/);
  assert.match(html, /btn\.addEventListener\('mouseenter',\(\)=>clearEventUnseenDot\(event\?\.id, btn\)\)/);
  assert.match(html, /clearEventUnseenDot\(event\?\.id, btn\); openDayEventDetail\(index\)/);
});
...
test('grocery state is shared through the groceries API with local fallback and participates in dashboard CTAs', () => {
  ...
  assert.match(html, /const GROCERY_SEEN_KEY='korculaGrocerySeen'/);
  assert.match(html, /function currentGrocerySeenKey\(\)/);
  assert.match(html, /return `\$\{GROCERY_SEEN_KEY\}:\$\{currentGroceryPerson\(\) \|\| 'anonymous'\}`/);
  assert.match(html, /function readGrocerySeenAt\(\)/);
  assert.match(html, /function markGroceriesSeen\(\)/);
  assert.match(html, /let activeGroceryUnseenItemIds=new Set\(\)/);
  assert.match(html, /function snapshotUnseenGroceryItems\(\)/);
  assert.match(html, /activeGroceryUnseenItemIds=new Set\(unseenGroceryItems\(\)\.map\(item=>item\.id\)\)/);
  assert.match(html, /function dismissUnseenGroceryItemDot\(id\)/);
  ...
  assert.match(html, /function openPrepPlanner\(\)\{[\s\S]*markGroceriesSeen\(\)/);
  assert.match(html, /updatedAt:new Date\(\)\.toISOString\(\)/);
  assert.match(html, /\.dashboard-unseen-badge \{[^}]*background: rgba\(41, 143, 184, \.25\);[^}]*box-shadow: 0 3px 8px rgba\(5,34,48,\.12\);/s);
...
test('people profiles live in an animated fit-content dashboard tab control', () => {
  ...
  assert.match(html, /class="profile-detail-row"[\s\S]*<svg class="profile-detail-icon" data-symbol="cart" aria-hidden="true"[\s\S]*<span id="profileGroceries">No groceries added yet\.<\/span>/);
  ...
  assert.match(html, /\$\{profileIcon\('cart'\)\}<span>\$\{escapeHtml\(groceryLine\(person\)\)\}<\/span>/);
  assert.match(html, /profileGroceries\.textContent=groceryLine\(name\)/);
  ...
  assert.doesNotMatch(html, /const groupDrivers=/);
  ...
  assert.doesNotMatch(html, /function isGroupDriver/);
  ...
  assert.match(html, /function hasDietaryPreference\(person\)\{ return dietaryLine\(person\)!=='No dietary preferences added yet\.'; \}/);
  assert.match(html, /function groceryItemsAddedBy\(person\)/);
  ...
  assert.doesNotMatch(html, /personSummaryIcon\('car','Group driver'/);
  assert.doesNotMatch(html, /car:'<path/);
  ...
  assert.match(html, /\.person-summary-main \{[^}]*display: inline-flex;[^}]*align-items: center;[^}]*gap: 8px/s);
