...
test('grocery card replaces trip prep with dietary preferences and a shared categorized grocery list', () => {
  ...
  assert.match(html, /<\/textarea><div class="dietary-confirmation-row"><button class="dietary-save-action" id="saveDietaryPreferences" type="submit">Save<\/button><span class="dietary-saved-message" id="dietarySavedMessage" hidden>Saved to your profile\.<\/span><button class="dietary-edit-action" id="editDietaryPreferences" type="button" hidden>Edit<\/button><\/div><\/div>/);
...
test('grocery state is shared through the groceries API with local fallback and participates in dashboard CTAs', () => {
  ...
  assert.match(html, /dietarySavedMessage\.hidden=!\(hasPreference&&!editing\)/);
  ...
  assert.match(html, /\.dietary-preference-card\.is-saved #dietaryNotes \{[^}]*height: 44px;[^}]*min-height: 44px;[^}]*resize: none;/s);
  assert.match(html, /\.dietary-saved-message \{[^}]*color: rgba\(255,255,255,\.72\);[^}]*font-size: 13px;/s);
  assert.match(html, /\.dietary-edit-action \{[^}]*opacity: 0;[^}]*pointer-events: none;/s);
  assert.match(html, /\.dietary-preference-card\.is-saved:hover \.dietary-edit-action, \.dietary-preference-card\.is-saved:focus-within \.dietary-edit-action \{[^}]*opacity: 1;[^}]*pointer-events: auto;/s);
  ...
  assert.match(html, /\.grocery-add-action, \.dietary-save-action, \.dietary-edit-action \{[^}]*border: 1px solid white;[^}]*background: white;[^}]*color: var\(--ink\);[^}]*border-radius: var\(--field-radius\);[^}]*height: 34px;[^}]*min-height: 34px;[^}]*box-sizing: border-box;[^}]*padding: 0 14px;/s);
  ...
  assert.match(html, /@media \(max-width: 760px\) \{[\s\S]*\.grocery-add-action, \.dietary-save-action, \.dietary-edit-action \{ height: 44px; min-height: 44px; border-radius: 18px; padding: 0 16px; \}/s);
