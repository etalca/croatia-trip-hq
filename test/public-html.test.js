...
test('people profiles live in an animated fit-content dashboard tab control', () => {
  ...
  assert.match(html, /profileSummaryLine\.textContent=arrivalCountdownLine\(name, board\)/);
  assert.match(html, /profilePhone\.innerHTML=phoneLine\(name\)/);
  assert.match(html, /profileRoom\.textContent=roomLine\(name\)/);
  assert.match(html, /profileDinner\.textContent=dinnerLine\(name, dinner\)/);
  assert.match(html, /profileDietary\.textContent=dietaryLine\(name\)/);
